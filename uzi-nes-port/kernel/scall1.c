...
int _open(char *name, int16 flag)
{
	...
	if (getmode(ino) == F_DIR && (flag == O_WRONLY || flag == O_RDWR)) {
...
void readi(register inoptr ino)
{
	...
	case F_REG:
		...
		toread = udata.u_count =
		    ...
		    udata.u_count :
		    min(udata.u_count,
			512 * (ino->c_node.i_size.o_blkno -
			       udata.u_offset.o_blkno) +
			(ino->c_node.i_size.o_offset -
			 udata.u_offset.o_offset));
		goto loop;
 ...
 loop:
		while (toread) {
			if ((pblk =
			     bmap(ino, udata.u_offset.o_blkno, 1)) != NULLBLK)
				bp = bread(dev, pblk, 0);
			else
				bp = zerobuf();
			...
			bcopy(bp + udata.u_offset.o_offset, udata.u_base,
			      (amount =
			       min(toread, 512 - udata.u_offset.o_offset)));
			brelse(bp);
...
void writei(register inoptr ino)
{
 ...
 loop:
		...
		while (towrite) {
			...
			if ((pblk =
			     bmap(ino, udata.u_offset.o_blkno, 0)) == NULLBLK)
